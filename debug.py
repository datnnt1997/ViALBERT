...
def create_instances_from_document(all_documents, document_index, max_seq_length, short_seq_prob,
    ...
    """
    document = all_documents[document_index]  # Lấy một document tại vị trí hiện tại

    # Tính số lượng token tối đa trừ ([CLS], [SEP], [SEP])
    ...
    if random.random() < short_seq_prob:
        # Sử dụng một tỉ lệ nhật định (vd: 10%) để sử dụng một sequence có độ dài ngắn hơn để giảm bớt sự chênh lệch
        # độ dài trong tập dữ liệu .
    ...
    while i < len(document):  # 从文档的第一个位置开始，按个往下看
        ...
        if i == len(document) - 1 or current_length >= target_seq_length:
            ...
            if current_chunk:  # 如果当前块不为空
                ...
                tokens, masked_lm_positions, masked_lm_labels = create_masked_lm_predictions(tokens, max_ngram,
                                                                                             masked_lm_prob,
